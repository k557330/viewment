export const leastSquares = (chartData) => {
    ...
    let priceAvg = 0
    for(let i = 0; i < chartData.length; i++) priceAvg += (chartData[i].price / 1000)
    const PRICE_AVG = priceAvg / chartData.length

    let a, b, tmp = 0, tmp2 = 0
    for (let i = 0; i < chartData.length; i++) {
        tmp += (i + 1 - TIME_AVG) * (chartData[i].price - PRICE_AVG)
        tmp2 += Math.pow(chartData[i].price - PRICE_AVG, 2)
    }
    a = tmp / tmp2
